...
public class OpenHashingTest {
    ...
    void constructorNullMapping() {
        assertThrows(RuntimeException.class, () -> new OpenHashing<Integer, String>(null));
    }

    @Test
    void insertOrUpdateNullKeyOrValue() {
        OpenHashing<Integer, String> h = new OpenHashing<>(k -> k);
    ...
    @Test
    void collisionAndChainingBehavior() {
        // mapping that forces all keys to the same bucket
        OpenHashing<Integer, String> h = new OpenHashing<>(k -> 0);
        ...
        assertEquals("v6", h.find(6));
        assertEquals("v7", h.find(7));

        // inserting a new key should be added to the same chain
    ...
    void rehashTriggerAndIntegrity() {
        // identity prehash; initial table length is 10 and threshold is 0.75 => rehash happens when size/len > 0.75
        OpenHashing<Integer, String> h = new OpenHashing<>(k -> k);
        ...
        // ensure updates still work after rehash
        h.insertOrUpdate(5, "five");
        assertEquals("five", h.find(5));
        assertEquals(8, h.size());
    }
