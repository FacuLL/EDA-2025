...
abstract public class AdjacencyListGraph<V, E> implements GraphService<V, E> {
    ...
    private V getSongArtist(Song song) {
        Collection<InternalEdge> internals = getAdjacencyList().getOrDefault(song, List.of());
        if (internals.isEmpty()) throw new IllegalArgumentException("Inconsistent song");
        for (InternalEdge internal : internals) {
            if (internal.edge instanceof WeightedEdge) {
                if (internal.target instanceof Artist) {
                    return internal.target;
                }
            }
        }
        throw new IllegalArgumentException("Inconsistent song");
    }

    @Override
    public GraphService<V, E> fansSubgraph(Integer popularThreshold) {
        if (!isDirected || !isWeighted || !isSimple) throw new IllegalArgumentException("El grafo debe ser simple, dirigido y con peso");
        ...
        for (V vertex : getVertices()) {
            if (vertex instanceof Fan) {
                Map<V, Integer> replays = new HashMap<>();
                Collection<InternalEdge> internals = getAdjacencyList().getOrDefault(vertex, List.of());
                for (InternalEdge internal : internals) {
                    if (internal.edge instanceof WeightedEdge edge) {
                        if (internal.target instanceof Song song) {
                            V artist = getSongArtist(song);
                            int actualReplays = replays.getOrDefault(artist, 0);
                            replays.put(artist, actualReplays + edge.getWeight());
                        }
                    }
                }
                replays.forEach((key, value) -> {
                    if (value >= popularThreshold) {
                        subgraph.addVertex(vertex);
                        subgraph.addVertex(key);
                        subgraph.addEdge(vertex, key, (E) new WeightedEdge(value));
                    }
                });
            }
        }
    ...
    public String toString() {
        ...
        for (Map.Entry<V, Collection<InternalEdge>> edge : getAdjacencyList().entrySet()) {
